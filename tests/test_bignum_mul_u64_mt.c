/**
 * @file    test_bignum_mul_u64_mt.c
 * @author  git@bayborodov.com
 * @version 1.0.0
 * @date    27.11.2025
 *
 * @brief   Динамический тест на потокобезопасность для bignum_mul_u64.
 *
 * @details
 *   Этот тест создает несколько потоков, каждый из которых многократно
 *   вызывает функцию `bignum_mul_u64` со своим, уникальным набором данных.
...
void* thread_func(void* arg) {
    ...
    for (int i = 0; i < NUM_ITERATIONS; ++i) {
        ...
        bignum_mul_u64_status_t status = bignum_mul_u64(&res, &data->a, data->b);

        if (status != BIGNUM_MUL_U64_SUCCESS) {
            data->ok = 0;
            break;
        }
...
int main(void) {
    printf("\n--- Starting MT test for bignum_mul_u64 ---\n");
    ...
    for (int i = 0; i < NUM_THREADS; ++i) {
        ...
        bignum_mul_u64_status_t st = bignum_mul_u64(&data[i].expected, &data[i].a, data[i].b);
        if (st != BIGNUM_MUL_U64_SUCCESS) {
    ...
    if (!all_ok) {
        fprintf(stderr, "--- MT test for bignum_mul_u64 FAILED ---\n");
        return 1;
    }

    printf("\n--- MT test for bignum_mul_u64 passed ---\n");
    return 0;
}
